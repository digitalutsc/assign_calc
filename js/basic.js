...
jQuery('.node__content').before("<p style='display: none; border: 2px solid; border-color: #d3d3d3;padding: 10px 4px 10px 4px;' id='messageText'></p> <div style='display: none' id='stepText'><b>Step Percentage</b>  <label id='switchButton' class='switch'>  <input type='checkbox' id='btnPercent'>   <span class='slider round'></span> </label></div> <br>");
if (count != -1) {
    jQuery(typeStep[count]).after("<button id='btnPrint' style='margin-left: 0' title='' class='button' onclick=''>Print Schedule</button> <a id='btnContact' href='/contact/assignment_planner_feedback'><button  title='' class='button' >Send Us Feedback</button></a>");
}
...
document.getElementById("btnPrint").onclick = function() {
    ...
    printWindow.document.write(document.getElementsByClassName('node__content')[0].innerHTML);
...
//bold hyperlinks
//hyperlinks=document.getElementsByClassName('node__content').getElementsByTagName('a');
var hyperlinks = jQuery('.node__content').find('a');
...
(function($, Drupal, drupalSettings) {
    if ($('body').is('.page-node-type-assignment') || $('body').is('.node--type-assignment')) {
